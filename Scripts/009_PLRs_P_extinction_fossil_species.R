...
ggsave(sensi_plot1, filename = "PLR_P(extinct)_vs_Magnitude_fossil_species_phylo_uncertainty_complete_trees_TACT.pdf", path = "Results/PLRs", width = 6, height = 6, device = cairo_pdf)
...
gt_tbl1 <- gt_tbl1 %>% tab_header(
  title = "Model: P(extinct) ~ Magnitude of body mass change",
  subtitle = "n = 827; 100 complete TACT trees")

gt_tbl1

#Save the table
gtsave(gt_tbl1, filename = "Table_Summ_stats_PLR_P(extinct)_vs_Magnitude_fossil_species.html", path = "Results/PLRs")
...
ggsave(sensi_plot2, filename = "PLR_P(extinct)_vs_Body_mass_fossil_species_phylo_uncertainty_complete_trees_TACT.pdf", path = "Results/PLRs", width = 6, height = 6, device = cairo_pdf) 

#Extract summary stats in a table
stats_PLR2 <- PLR2$all.stats
stats_PLR2 <- stats_PLR2 %>% mutate_if(is.numeric, round, digits=8)
...
#tranform list AICc in data frame
...
gt_tbl2 <- gt_tbl2 %>% tab_header(
  title = "Model: P(extinct) ~ Body mass (kg)",
  subtitle = "n = 827; 100 complete TACT trees")

gt_tbl2

#Save the table
gtsave(gt_tbl2, filename = "Table_Summ_stats_PLR_P(extinct)_vs_Body_mass_fossil_species.html", path = "Results/PLRs")
